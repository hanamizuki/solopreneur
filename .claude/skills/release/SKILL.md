...
description: |
  ...
  Optionally creates one GitHub Release on `solopreneur-core--v<new>` for
...
```bash
PLUGINS=(core ios android nextjs python llm)
for p in "${PLUGINS[@]}"; do
  PLUGIN="solopreneur-$p"
  LAST_TAG=$(git tag -l "${PLUGIN}--v*" --sort=-v:refname | head -1)
  if [ -z "$LAST_TAG" ]; then
    echo "=== $PLUGIN: NO TAG YET (first release) ==="
    git log --oneline -- "plugins/$p/"
    continue
  fi
  CHANGED=$(git diff --name-only "$LAST_TAG"..HEAD -- "plugins/$p/" | wc -l | tr -d ' ')
  if [ "$CHANGED" -gt 0 ]; then
    echo "=== $PLUGIN: $CHANGED files changed since $LAST_TAG ==="
    git log --oneline "$LAST_TAG"..HEAD -- "plugins/$p/"
  else
    echo "=== $PLUGIN: no changes since $LAST_TAG ==="
  fi
done
```
...
for p in "${BUMPED[@]}"; do
  ...
  git tag -a "solopreneur-${p}--v${VERSION}" \
    -m "solopreneur-${p} v${VERSION}: <one-line>"
done
```
...
- New sub-plugin shipped (e.g. `solopreneur-rust` joins the family)
...
1. The Release attaches to **one** tag — use `solopreneur-core--v<new>` as
   the umbrella, even if core didn't change much. (Repo's "Latest release"
   surface should look like a single coherent version, not six per drop.)

   If core wasn't bumped this cycle, attach to whichever plugin's tag best
   represents the milestone, but warn the user that the umbrella tag is
   typically core.
...
3. Create:
   ...
   ```bash
   gh release create "solopreneur-core--v<new>" \
     --title "v<new> — <theme>" \
     --notes-file <(cat <<'EOF'
