...
- **[Codex GitHub bot](https://github.com/apps/chatgpt-codex-connector)**: optional. It is the preferred GitHub final gate for Claude-hosted `/greenlight` PR runs, with Codex CLI as the local fallback. Its formal reviews identify the reviewed commit; reaction-only clean signals are accepted only for the current trigger while the PR head remains unchanged.
- **[Cursor Bugbot](https://docs.cursor.com/bugbot)**: optional. It is the preferred GitHub final gate for Codex-hosted PR runs once its real clean signal is calibrated; Claude CLI is the local fallback and remains available when Bugbot cannot produce a usable verdict.
