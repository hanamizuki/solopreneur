...
- **[gstack review](https://github.com/garrytan/gstack/tree/main/review)**: recommended optional reviewer for the approved [cross-host reviewer contract](./docs/spec/2026-08-10-greenlight-cross-host-review-contract.md). The current Claude `/greenlight` L-size phase still names the unqualified `/review`, which Claude Code 2.1.223+ reserves as its bundled `/code-review` alias; installing gstack alone therefore does not prove that Greenlight invoked it. The planned adapter resolves the logical `gstack-review` recipe to the current-session invocation token, then verifies that its provider provenance is actually gstack.
- **[Codex GitHub bot](https://github.com/apps/chatgpt-codex-connector)**: optional for the current Claude-oriented `/greenlight` PR runtime, which documents it first in the configured ladder. The built-in empty-ladder path seeds `@codex review` only when there is no reviewer activity or cache and no authenticated Codex CLI candidate. Under the approved host-aware contract, Codex CLI becomes the Claude-host V1 default; the bot can become a conditional candidate only through explicit selection or a user-configured ladder after its review protocol and target-safe permission boundary pass L14. As an OpenAI reviewer, it cannot independently gate a Codex-host run.
