...
- [ ] Implement frozen internal rosters: Claude requires bundled Simplify; Codex requires native Codex review; both invoke bundled `specialist-review` and every available Superpowers, gstack, and Ponytail capability. Codex may add Claude Simplify only through an explicitly matched Claude profile. Invocation failure must remain visible rather than becoming an absent dependency.
