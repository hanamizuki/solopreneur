...
- [ ] Implement frozen internal rosters: Claude requires bundled Simplify; Codex requires native Codex review; both invoke bundled `specialist-review` and every available Superpowers, gstack, and Ponytail capability. Codex may add Claude Simplify only through an explicitly matched Claude profile. Before either host claims conformance, its real bundled-specialist adapter must pass the paired clean and seeded-stack-finding L03 branch with exact-target, parser, provenance, transport, and no-mutation evidence. The Claude engine must pass L04 direct-native binding, and the Codex engine must pass L05 with `host_tool + host_native`, active-profile, expected/observed capability, provider, parser, and wrong-context controls. Later transient specialist invocation failure remains visible as `incomplete-review` rather than becoming an absent dependency.
