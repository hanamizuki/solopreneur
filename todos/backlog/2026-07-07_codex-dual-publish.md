...
- [x] Approve the cross-host Greenlight reviewer contract and its 15-case acceptance matrix: internal advisors versus final gate, host-native minimums, available-to-must-run optional integrations, host-specific defaults, anti-shopping fallback, and exact-target pass evidence. See `docs/spec/2026-08-10-greenlight-cross-host-review-contract.md`.
- [ ] Extend the reviewer registry with internal/external phase, host, expected and observed capability provenance, implementation group, parser identity, exact-target binding, remote settlement evidence, and host-specific gate eligibility. Add the parent-only GitHub App permission-authority broker and its supported credential profiles; ordinary `gh` authentication is not authority. Keep current runtime behavior explicit until its engine migration lands.
- [ ] Implement frozen internal rosters: Claude requires bundled Simplify; Codex requires native Codex review; both invoke bundled `specialist-review` and every available Superpowers, gstack, and Ponytail capability. Codex may add Claude Simplify only through an explicitly matched Claude profile. Before either host claims conformance, its real bundled-specialist adapter must pass the paired clean and seeded-stack-finding L03 branch with exact-target, parser, provenance, transport, and no-mutation evidence. Before any optional Superpowers, gstack, or Ponytail recipe is promoted on a host, that exact recipe/host tuple must pass its own paired parser-valid clean and recipe-specific seeded-finding L03 branch; discovery or invocation alone is insufficient, while a missing optional recipe does not block host conformance. The Claude engine must pass L04 direct-native binding, and the Codex engine must pass L05 with `host_tool + host_native`, active-profile, expected/observed capability, provider, parser, and wrong-context controls. Later transient specialist or promoted-optional invocation failure remains visible as `incomplete-review` rather than becoming an absent dependency.
- [ ] Implement host-aware CLI-first final-gate defaults and migration: Claude Code defaults to Codex CLI; Codex defaults to Claude CLI. Codex bot and Bugbot remain ineligible until both their exact-mode review protocol and parent-authoritative target-safe App permission boundary are promoted; promotion only permits explicit selection or a user-configured ladder and must not silently reorder either V1 default. A remote finding remains non-actionable until all verified-blocking windows settle; only then may a fixer mutate the target and require mandatory same-primary re-review. A valid new finding remains sticky. Only an availability-class failure on that re-review may use the fallback, with a recorded reason and a full review of the exact post-fix target.
- [ ] Validate the Claude CLI structured review recipe against clean, seeded-bug, malformed-output, stale-target, auth-failure, timeout, and no-mutation fixtures. Exit code zero and free-form prose are never clean.
- [ ] Validate the Codex CLI default gate through L13 with paired clean and seeded-bug fixtures under the exact-target contract. Probe the actual Codex bot separately through L14 with an actually supported parent-only permission-authority profile; ordinary `gh`, a substitute App, or missing vendor authority can satisfy only the no-promotion branch. Only a target-safe positive branch may promote explicit or user-configured use, while an unsafe or failed bot probe leaves it ineligible and does not block the Claude-host CLI-default engine. Internal Codex review cannot substitute for the CLI gate, and historical bot success cannot substitute for L14.
- [ ] Calibrate the actual Bugbot App with paid opt-in clean and seeded-bug PR fixtures plus an actually supported parent-only permission-authority profile. Until identity, completion, positive zero-finding signal, finding parser, exact-head binding, and the target-safe App permission boundary all pass, treat Bugbot as unavailable and use Claude CLI. Ordinary `gh`, a substitute App, or missing vendor authority can satisfy only the no-promotion branch. Passing the positive branch only promotes Bugbot as an explicit or user-configured candidate; Claude CLI remains the Codex-host V1 default.
- [ ] Make Greenlight reviewer independence executable: require an eligible final gate for the exact post-mutation target; invalidate old clean evidence after any mutation; return structured halt rather than clean when no gate is available; keep objective verification separate.
