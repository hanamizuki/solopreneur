...
- [x] Approve the parity-first cross-host Greenlight contract: internal advisors versus final gate, host-native minimums, installed-to-invoked optional integrations, host-aware GitHub and CLI gates, anti-shopping fallback, and serialized reviewed-head evidence. See `docs/spec/2026-08-10-greenlight-cross-host-review-contract.md`.
- [ ] Extend the reviewer registry with internal/external role, supported host, provider family, capability provenance, availability, invocation adapter, clean-signal shape, and reviewed-head metadata. Keep current Claude behavior explicit until the shared engine migration lands.
- [ ] Implement the internal roster: Claude requires Simplify; Codex requires native Codex review; both invoke bundled `specialist-review` and every callable Superpowers, gstack, and Ponytail capability. Codex may add Claude Simplify only through the explicitly matched Claude profile. A failed invocation remains visible and cannot be rewritten as unavailable.
- [ ] Implement the serialized final-gate loop. Freeze PR head `H`, prohibit mutation while review is in flight, accept only current-trigger evidence for `H`, and invalidate the old clean result after any new commit. A finding requires a fix, verification, a new head, and another review by the selected gate. Fallback is allowed for unavailability, quota, timeout, or invalid evidence, never merely because the gate found an issue.
- [ ] Preserve the Claude-host product path with Codex Bot as the preferred GitHub gate and Codex CLI as local fallback. Validate formal-review commit binding, reaction-only clean on the current trigger, clean and seeded-finding fixtures, stale-head rejection, timeout, and malformed evidence.
- [ ] Add the Codex-host counterparts: calibrate Bugbot's real clean and finding signals, and add Claude CLI as local fallback through the matching Claude profile. Until Bugbot can prove a positive clean signal, use Claude CLI rather than treating silence as clean.
- [ ] Make reviewer independence executable: the selected final gate must be outside the coordinating host's provider family; same-provider reviewers may still contribute advisory findings. Return structured non-pass rather than clean when no eligible gate remains, and keep objective verification separate from reviewer clean.
...
- [ ] Close the Autopilot-critical dependencies only: support `plan-review internal` and a safe `merge-pr` seam that performs consolidation before final review or reruns Greenlight and CI after any resulting commit.
