...
- [x] Measure whether Codex can read the shipped `greenlight/SKILL.md` and drive Phase 3 end to end against a real PR. Two A2 runs completed; run 2 reached the seeded-finding → fix → re-review → clean terminal path. ✅ PRs #164/#167 and `docs/spec/2026-08-10-codex-greenlight-port.md`
- [x] Add Claude CLI as an independent gate recipe with host-family enforcement and ambient profile inheritance. ✅ PRs #162/#163
- [x] Ship the runtime slice of Codex `/greenlight external` V1: unattended pull-request mode, objective verification, and structured terminal reporting from one skill body. Publication still waits on the registry and filtered view. ✅ PRs #162/#165
- [ ] Internal review on Codex — no longer blocked by the spawn question. An independent 2026-08-11 reproduction on CLI 0.147.0 showed the interactive TUI *and* `codex exec` both create a real `marketer` subagent thread from the router skill: one `spawn_agent` per run with `agent_type="marketer"` and `fork_turns="none"`, a child rollout naming the root as `parent_thread_id` at depth 1, and a task completion on both sides. The 0.144.3 no-spawn result was a version artifact, not a headless-harness limit. What still gates Phase 1 is scope, not platform: only `marketer` has a Codex agent today, so the reviewer agents and their routers must exist before internal review can run. A second probe the same day sized that work: a spawned child receives the full namespaced skill catalog as a plaintext developer message and can read any skill body from the path the catalog gives it, so a reviewer needs an agent definition and a brief — not a new skill-access mechanism. See [pilot findings](../../docs/spec/2026-07-15-codex-dual-publish-pilot-findings.md).
