...
- [ ] Resolve the marketer v2 vertical slice first: one marketer TOML, `codex-agents-bootstrap`, and `using-marketer`; update the authoritative pilot documents with accepted current evidence. The corrected final-byte local R02/R07/R08/R09 matrix passed; the final git-ref matrix and remaining merge gates are pending.
...
- [ ] In the marketer v2 slice, install via local marketplace and git `@ref`; verify skill-triggered agent spawning on every claimed Codex surface without claiming skill parity. The corrected final-byte local install and matrix passed; the published final git-ref install and matrix remain pending.
