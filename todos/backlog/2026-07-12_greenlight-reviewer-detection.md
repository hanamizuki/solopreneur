...
**3. 活動 ≠ liveness**
活動只證明「曾運作」，不證明「現在可用」——低流量 repo 的歷史紀錄會永遠看起來新鮮；當下可用性只有 trigger handshake（如 Codex 👀）能證明。偵測定位為**縮小候選 + 提示**，不做可用性判準。第一版不做 stale 判斷，只顯示 `last_seen_in_sample`；要做 stale 必須先寫出可執行定義（門檻、比較基準、觸發行為）＋驗收案例。
