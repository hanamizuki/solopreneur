...
- **Unset — Claude Code:** run the complete flow below unchanged.
- **Set — Codex V1:** parse the arguments, then continue only when `internal`
  is present as a whole token. Run Step 0, Stages 1–2, and Resolution R1–R2;
  never run Stage 3, R3, or R4, and never modify the reviewed document. If
  `internal` is absent, emit
  `Verdict: Needs revision — Codex V1 supports only /plan-review internal`
  and stop before dispatching reviewers or writing anything.

On Codex, Stage 1c uses an available matching custom agent when one exists and
otherwise the built-in `explorer`. Start either with `fork_turns="none"`;
Codex rejects a named agent that inherits full parent history. If this thread
cannot spawn another agent, perform the same evidence-backed technical vetting
inline and label that degradation in Stage 1; never narrate a dispatch that did
not happen. This keeps the unattended contract available from an Autopilot
worker even when that worker is already at the configured subagent-depth limit.
...
usage, or the APIs involved are fundamental knowledge. On Codex, an available
web-search tool may query the same primary official documentation instead.
Skipping the lookup skips only 1b — 1c still runs.

### 1c. Dispatch expert subagents

Dispatch the subagents for the detected platforms **in parallel**. On Claude
Code, an unavailable platform-specific type falls back to `general-purpose`.
On Codex, it falls back to the built-in `explorer`; dispatch either reviewer
with `fork_turns="none"`. If spawning is unavailable or rejected at the current
depth, run the review inline as required by the Codex host profile. The prompt
and output contract stay the same on every path.
