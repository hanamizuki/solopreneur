...
**Source/package layout:** [symmetric plugin layout](./2026-08-12-symmetric-plugin-layout.md)
...
- Maintain every shared skill once under `skills/<name>/`.
...
- No hand-maintained skill copies inside generated plugin packages.
...
3. **Non-skill directories** — keep vendoring metadata and shared helpers under
   `src/<plugin>/`; keep all skill bodies under `skills/<plugin>/`.
...
8. **Skill layout** — keep the single skill source under the repository-root
   `skills/` tree and generate both platform packages from it.
...
| --- | --- | --- |
| `skills/<n>/**` | both | hand-maintained |
| `src/<n>/agents/<n>.md` | Claude Code | hand-maintained |
| `src/<n>/agents/<n>.toml` | Codex source | hand-maintained |
| `src/<n>/plugin.json` | both platforms and version source | hand-maintained |
| `skills-compatibility.json` | both publication policy | hand-maintained |
| `plugins/claude/<n>/**` | Claude Code install roots | generated |
| `plugins/codex/<n>/**` | Codex filtered install roots | generated |
...
| `src/<n>/vendor/` | repository maintenance | generated by vendor sync |
| `src/solopreneur/shared/` | both | hand-maintained |
...
emits only plugin names with at least one included Codex skill. Each final
source points to `./plugins/codex/<name>`. During the tagged-release bridge it
points to a byte-identical `.codex/plugins/<name>` compatibility copy.
Installation and authentication policies are explicit, and category comes
from the same interface overlay used by the plugin manifest.
...
2. **Generation drift** — regenerate both platform package trees and fail on
   any diff.
...
5. **Install integration** — install all seven Claude plugins and every entry
   in the filtered Codex marketplace into throwaway homes. The cached
   bootstrap-to-agent round trip runs
