...
Merge PR, and Autopilot Codex V1 are included as degraded capabilities on Codex
exec, TUI, and App
...
| 9. Autopilot Codex V1 | Complete — this real PR exercised inline planning, Plan Review internal, executable verification, Greenlight unattended, exact-head CI, atomic merge, parent verification, cleanup, structured reporting, and the built-in worker fallback | The live single-PR lifecycle is accepted without a specialist agent |
...
block Codex publication. The dependency closure and Autopilot Codex V1 are accepted, so the next core slice
is rollout row 10: Greenlight's remaining modes before Autopilot multi-PR waves.
