# MVP PRD Visual Confirmation — Approved Design

Date: 2026-05-17

Insert a new **PRD visual confirmation** step into the `/mvp` orchestrator
skill, between the existing Brainstorming step and the Template lookup
step, then renumber the rest of the flow and update all cross-references.

Step-number mapping (applied consistently everywhere):
...
## B. New section, inserted between `## Step 1: Brainstorming` and the (now renumbered) `## Step 3: Template lookup`
...
## C. Step 3 (Template lookup) — append this decision sub-step to the per-platform loop (after the existing match / no-match / partial-match handling, before "Recovery paths")
...
## D. Step 4 (Writing the plan) — extend the inputs passed to `writing-plans`

Add a sentence to the existing "pass the records" guidance:
...
## E. Step 5 (Execute) — add this subsection before the existing 5b dispatch / commit-policy content
