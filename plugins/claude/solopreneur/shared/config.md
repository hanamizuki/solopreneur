...
functions of its own. `skills/solopreneur/merge-pr/SKILL.md` holds the
canonical copy; the block below reproduces it and must be updated from there,
...
Two consequences, and the resolution block above is what satisfies both:
...
```bash
grep -rl "# --- solopreneur config helpers" skills/solopreneur/
```

**`tests/config-helpers.test.mjs` fails when a consumer re-inlines the
helpers.** For each of the five it asserts the marker block covers the packaged,
canonical-authoring, and detached paths, then sources the resolved
`config.sh`. It defines no functions — in any of bash's three
...
```bash
grep -rlF 'url="${url#git@}"' skills/solopreneur/ | while read -r f; do grep -LF '# --- solopreneur config helpers' "$f"; done
```
...
Preview's **library** delivery — the default — resolves this file for its
content root, collections and include set, and writes a `preview.json`
sidecar per item; the local build renders that same tree under
`<root>/library/`. The explicit Claude Code Vercel workflow resolves the
same file and publishes the same tree. Only **ephemeral** delivery (explicit
temporary intent, or the fallback when no v2 config resolves) neither reads
nor writes it and creates no sidecar.
