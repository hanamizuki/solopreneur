...
const SKILLS = path.join(SHARED, '..', '..', '..', 'skills', 'solopreneur');
...
for (const skill of CONSUMERS) {
  test(`${skill} sources config.sh instead of defining the helpers`, () => {
    ...
    // Every candidate path, not merely the word `source`: a block that resolves
    ...
    assert.ok(block.some((l) => l.includes('/../../../src/solopreneur/shared/config.sh')),
      `${skill}/SKILL.md must resolve the canonical src/ helper while authoring`);
