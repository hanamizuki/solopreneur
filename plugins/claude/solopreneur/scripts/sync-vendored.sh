...
#
# Canonical location: src/solopreneur/scripts/sync-vendored.sh.
# Sub-plugins reach it via relative symlinks at src/<plugin>/scripts/sync-vendored.sh
...
if [[ ! -f "$MANIFEST" ]]; then
  ...
  echo "       Run a vendored sub-plugin source, for example:" >&2
  echo "         ./src/android-dev/scripts/sync-vendored.sh" >&2
  exit 1
fi
...
echo "  git -C $REPO_ROOT diff --stat -- skills/$PLUGIN_NAME src/$PLUGIN_NAME/vendor"
