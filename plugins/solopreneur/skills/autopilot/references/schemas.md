...
prs:
  - id: pr1                           # Unique identifier (used in depends_on references)
    ...
    select: coderabbit,codex-bot      # optional — reviewers for /greenlight; omit to use whatever acts on the repo
    gate: codex-bot                   # optional — whose clean pass ends the review loop; omit to use fallback_order
...
4. **List file paths**: so overlapping PRs are visible at planning time, and so
   `plan-review` has concrete paths to cross-check the plan's claims against
