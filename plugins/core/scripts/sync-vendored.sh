...
#
# Canonical location: plugins/core/scripts/sync-vendored.sh.
# Sub-plugins reach it via relative symlinks at plugins/<plugin>/scripts/sync-vendored.sh
# so PLUGIN_DIR resolves to the calling plugin's root, not core's.
#
...
if [[ ! -f "$MANIFEST" ]]; then
  ...
  echo "       If you ran it from plugins/core/scripts/ directly, switch to a sub-plugin" >&2
  echo "       (e.g. plugins/android, plugins/ios, plugins/designer) and run from there:" >&2
  echo "         cd plugins/android && ./scripts/sync-vendored.sh" >&2
  exit 1
fi
